...
BEGIN
    ...
    DECLARE @lote INT;
    SELECT @lote = ISNULL(MAX(lote_execucao), 0) + 1
    ...
    INSERT INTO violacao.ft_venda_violacao
        (lote_execucao, pedido_id, item_pedido_id, motivo, campo, valor_encontrado)
    SELECT @lote, ip.pedido_id, ip.item_pedido_id,
    ...
    SELECT @lote AS lote_execucao, COUNT(*) AS total_violacoes
    FROM violacao.ft_venda_violacao
    WHERE lote_execucao = @lote;
END;
GO
